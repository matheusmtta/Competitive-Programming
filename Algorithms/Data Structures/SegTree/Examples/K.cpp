...
//Find insersecting segments in array ex: xyyzxz
//z..z is intersecting the segment x...x
...
int main(){	
	...
	segTree <int> open(zeros, 2*n);
	segTree <int> closed(zeros, 2*n);
	...
	for (int i = 0; i < 2*n; i++){
		if (visited[arr[i]] == -1){
			...
			open.update(visited[arr[i]], -1);
			ans[arr[i]] = open.query(visited[arr[i]], i) + closed.query(visited[arr[i]], i);
			closed.update(i, 1);
		}
	}
