...
//range maximum queries with range updates
...
struct segTree {
	...
	void prop(int v, int l, int r){
		...
		segtr[nxt] = combine(lazy[v], segtr[nxt]);
		segtr[nxt+1] = combine(lazy[v], segtr[nxt+1]);
		lazy[nxt] = combine(lazy[v], lazy[nxt]), lazy[nxt+1] = combine(lazy[v], lazy[nxt+1]);
		lazy[v] = 0;
	}
	...
	T update(int v, int l, int r, int lx, int rx, T val){
		...
		if (l >= lx && r <= rx) {
			segtr[v] = combine(val, lazy[v]);
			lazy[v] = combine(val, lazy[v]);
			return segtr[v];
		}
